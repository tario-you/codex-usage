...
test('browser request schema preserves owner data, blocks direct clients, and allows only one queued click per device', { skip: !available && 'PostgreSQL binaries required' }, () => {
  ...
  try {
    ...
    sql(readFileSync(new URL('../supabase/migrations/20260920220000_browser_login_method.sql', import.meta.url), 'utf8'))
    ...
    sql(insert)
    assert.equal(sql('select login_method from codex_browser_launches;'), 'email')
    sql("update codex_browser_launches set login_method='google';")
    assert.throws(() => sql("update codex_browser_launches set login_method='invalid';"), /check constraint/)
