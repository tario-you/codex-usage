...
export function AccountBrowserLink({ account, session, children }: {
  account: DashboardAccountRow; session: Session; children: ReactNode
}) {
  ...
  async function launch(device: BrowserDevice) {
    ...
    try {
      const { requestId } = await call('', { accountId: account.id, deviceId: device.id })
