-- #31: the history RPC returned one row per distinct snapshot timestamp. Once
-- every saved login synced on its own clock (sync --all, 2026-09-09) a 7-day
-- range held 18,000 timestamps, the browser's 1000-row cap kept the oldest
-- 1000, and the chart ended on Sep 9 02:56 while the table synced live.
-- Points are now time buckets sized to the range; each point still reads
-- every accessible account's latest snapshot at or before it, keeps the
-- weekly-window rule of 20260716093000 (the 10080-minute window in either
-- slot, else nothing), and the newest bucket ends now, so "Updated" names the
-- newest snapshot.

drop function if exists public.list_dashboard_weekly_usage_history(timestamptz);
...
revoke all on function public.list_dashboard_weekly_usage_history(timestamptz, integer)
  from public;
grant execute on function public.list_dashboard_weekly_usage_history(timestamptz, integer)
  to authenticated;
